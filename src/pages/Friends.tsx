    ...
    try {
      ...
      try {
        ...
        if (writeErr.code === 'permission-denied') {
          setError(
            'Failed to send friend request: Missing or insufficient permissions. Please make sure you have updated your Firebase security rules.'
          );
        } else {
  ...
  const removeFriend = async (user: User) => {
    ...
    try {
        ...
        try {
          ...
          if (currentFriends.includes(currentUser.uid)) {
            ...
            const newFriends = currentFriends.filter(
              (friendId: string) => friendId !== currentUser.uid
            );
          ...
          console.error(
            'Error details:',
            alternativeUpdateError.code,
            alternativeUpdateError.message
          );
        }
      }
      ...
      if (otherUserUpdateSuccessful) {
        ...
        setSuccess(
          `${user.displayName} removed from your friends, but there was an issue updating their friends list. They may still see you as a friend.`
        );
      }
    } catch (err: any) {
      ...
      try {
        ...
        if (currentUserDoc.exists() && otherUserDoc.exists()) {
          ...
          const currentUserHasFriend =
            currentUserData.friends && currentUserData.friends.includes(user.uid);
          const otherUserHasFriend =
            otherUserData.friends && otherUserData.friends.includes(currentUser.uid);
