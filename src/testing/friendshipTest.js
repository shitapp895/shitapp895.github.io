...
const testFriendRemoval = async (yourUid, friendUid) => {
  try {
    ...
    try {
      ...
      await firestore.collection('users').doc(yourUid).update({
    ...
    if (!yourFriendsAfter.includes(friendUid) && !friendFriendsAfter.includes(yourUid)) {
      ...
      console.log('❌ TEST FAILED: Friend was removed from your list but you are still in their list');
    } else if (yourFriendsAfter.includes(friendUid) && !friendFriendsAfter.includes(yourUid)) {
      console.log('❌ TEST FAILED: You still have friend in your list but they don\'t have you in their list');
    } else {
